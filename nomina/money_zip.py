...
class MnyToZipConverter:
    ...
    def check_tools(self):
        ...
        for tool in tools:
            if shutil.which(tool):
                ...
                ok = False
        if not ok:
            self.negative("You might want to install https://github.com/mdbtools/mdbtools on your system")

    def create_yaml_declaration(self, mny_file, yaml_file):
        """Create YAML declaration file."""
        try:
            mdb_ver = subprocess.check_output(["mdb-ver", mny_file], universal_newlines=True).strip()
    ...
    def export_tables_to_json(self, mny_file, output_dir):
        ...
        tables = subprocess.check_output(["mdb-tables", "-1", mny_file], universal_newlines=True).split()
    ...
    def run(self):
        parser = argparse.ArgumentParser(description="Convert Microsoft Money files to ZIP")
        parser.add_argument("-ct", "--checktools", action="store_true", help="Check if required tools are installed")
        parser.add_argument("-d", "--debug", action="store_true", help="Enable debug output")
        parser.add_argument("--keep", action="store_true", help="Keep the temporary files after conversion")
        parser.add_argument("--version", action="version", version=f"Version: {self.VERSION}")
        parser.add_argument("-o", "--output", help="Specify the output directory - default: /tmp/[basename]_money_dump")
        parser.add_argument("mny_file", nargs="?", help="Microsoft Money file to convert")
